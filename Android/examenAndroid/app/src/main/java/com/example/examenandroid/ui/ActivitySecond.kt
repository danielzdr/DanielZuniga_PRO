...
class ActivitySecond : AppCompatActivity(), AdapterView.OnItemSelectedListener {
    ...
    private fun acciones() {
        ...
        binding.textMatricula.text = " ${coche?.matricula}"
        binding.textFecha.text = " ${coche?.anioMatriculacion}"
        binding.textAuto.text="${coche?.autonomia}"
        val imagen=intent.getIntExtra("diesel",2)
        binding.imagenTipo.setImageResource(imagen)
    }
