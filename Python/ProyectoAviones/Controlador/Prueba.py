...
flujo_aterrizaje = []  # IDs de vuelos pendientes de aterrizar
flujo_despegue = []    # IDs de vuelos pendientes de despegar
...
def crear_vuelo(id_, tipo, tiempo, prioridad, combustible, estado):
    ...
    vuelo = {
        ...
        "tiempo": tiempo,              # int: eta/etd en minutos simulados
        ...
        with open(ruta_fichero, mode="r", encoding="utf-8") as f:
            for num_linea, linea in enumerate(f, 1):
                ...
                try:
                    ...
                    # Crear vuelo para estructura activa
                    ...
                    # Agregar a las estructuras globales MANTENIENDO EL ESTADO ORIGINAL
                    vuelo_global = {
                        'id': id_,
                        'tipo': tipo,
                    ...
                    # Solo agregar a los flujos si está EN_COLA
                    if estado == 'EN_COLA':
                        if tipo == 'ATERRIZAJE':
                            flujo_aterrizaje.append(id_)
                        else:
                            flujo_despegue.append(id_)
                        print(f"✓ Cargado vuelo {id_} - {tipo} - EN_COLA")
...
def asignar_vuelo_inicial(id_vuelo, tipo):
    ...
    for pista_id, estado_pista in pistas_estado.items():
        ...
        if estado_pista['estado'] == 'LIBRE' and pista['habilitada']:
...
def agregar_vuelo_manual():
    ...
    if estado == 'EN_COLA':
        ...
        registrar_evento(f"EN_COLA id_vuelo={id_vuelo} tipo={tipo}")
    elif estado == 'ASIGNADO':
        # Intentar asignar automáticamente
        if not asignar_vuelo_inicial(id_vuelo, tipo):
            # Si no se pudo asignar, poner en cola
            nuevo_vuelo['estado'] = 'EN_COLA'
            if tipo == 'ATERRIZAJE':
                flujo_aterrizaje.append(id_vuelo)
            else:
                flujo_despegue.append(id_vuelo)
            registrar_evento(f"EN_COLA id_vuelo={id_vuelo} tipo={tipo} (falló asignación inicial)")
    elif estado == 'COMPLETADO':
        vuelos_completados.append({
            'id': id_vuelo,
            'tipo': tipo,
    ...
    print(f"✓ Vuelo {id_vuelo} agregado correctamente - Estado: {estado}")
...
def actualizar_combustible():
    ...
    for vuelo_id in flujo_aterrizaje[:]:  # Usar copia para evitar problemas al modificar
        ...
        if vuelo and vuelo['combustible'] is not None:
            vuelo['combustible'] -= 1
        ...
        if estado['estado'] == 'OCUPADA' and estado['fin_ocupacion'] <= reloj:
            ...
            if vuelo:
                ...
                vuelos_completados.append({
                    'id': vuelo_id,
                    'tipo': vuelo['tipo'],
                    'inicio': estado['fin_ocupacion'] - pista['tiempo_uso'],
                    'fin': estado['fin_ocupacion'],
                    'pista': pista_id
                })
                
                # Quitar de flujos si aún están allí (no deberían estar)
...
def comparar_prioridad(vuelo_id1, vuelo_id2):
    ...
    if vuelo1['tipo'] == 'ATERRIZAJE' and vuelo2['tipo'] == 'ATERRIZAJE':
        if vuelo1['combustible'] != vuelo2['combustible']:
            return vuelo1['combustible'] - vuelo2['combustible']
    
    # 3. Mayor atraso primero
    atraso1 = reloj - (vuelo1['eta'] if vuelo1['tipo'] == 'ATERRIZAJE' else vuelo1['etd'])
    atraso2 = reloj - (vuelo2['eta'] if vuelo2['tipo'] == 'ATERRIZAJE' else vuelo2['etd'])
    if atraso1 != atraso2:
...
def asignar_pistas():
    ...
    # Ordenar según política de prioridad
        ...
        if todos_vuelos:
            ...
            if vuelo:
                ...
                pistas_estado[pista_id] = {
                    'estado': 'OCUPADA',
                    'vuelo': vuelo_id,
                    'fin_ocupacion': reloj + pista['tiempo_uso']
                }
                ...
                registrar_evento(f"ASIGNACION id_vuelo={vuelo_id} pista={pista_id} tipo={vuelo['tipo']} duracion={pista['tiempo_uso']}min")
...
def mostrar_estado_actual():
    ...
    vuelos_asignados = [v for v in vuelos if v['estado'] == 'ASIGNADO']
    if vuelos_asignados:
...
def mostrar_vuelos_detallados():
    ...
    en_cola = [v for v in vuelos if v['estado'] == 'EN_COLA']
    asignados = [v for v in vuelos if v['estado'] == 'ASIGNADO']
    ...
    for vuelo in vuelos:
        ...
        prioridad_texto = {0: "Normal", 1: "Alta", 2: "🆘 EMERGENCIA"}[vuelo['prioridad']]
        estado_icono = {
            'EN_COLA': '⏳',
            'ASIGNADO': '🎯', 
            'COMPLETADO': '✅',
        ...
        print(f"{estado_icono} {tipo_icono} {vuelo['id']}: {vuelo['tipo']} | {prioridad_texto} | Estado: {vuelo['estado']}")
        
        if vuelo['tipo'] == 'ATERRIZAJE':
            atraso = max(0, reloj - vuelo['eta'])
            print(f"   ETA: {vuelo['eta']} | Atraso: {atraso}min | Combustible: {vuelo['combustible']}min")
        else:
            atraso = max(0, reloj - vuelo['etd'])
            print(f"   ETD: {vuelo['etd']} | Atraso: {atraso}min")
        
        # Mostrar información de pista si está asignado
        if vuelo['estado'] == 'ASIGNADO':
            for pista_id, estado_pista in pistas_estado.items():
...
def mostrar_menu_principal():
    ...
    print("\n" + "="*60)
    print("🚀 SISTEMA DE GESTIÓN DE TRÁFICO AÉREO")
    print("="*60)
...
def main():
    ...
    global reloj, vuelos, flujo_aterrizaje, flujo_despegue, eventos_log, vuelos_completados
    ...
    vuelos_activos = cargar_vuelos_desde_txt(ruta_fichero, {})
    ...
    print(f"• EN_COLA: {en_cola} | ASIGNADOS: {asignados} | COMPLETADOS: {completados}")
    ...
    while True:
        ...
        if opcion == '1':
            ...
            if eventos_log:
                for evento in eventos_log[-15:]:  # Mostrar últimos 15 eventos
                    print(evento)
            else:
